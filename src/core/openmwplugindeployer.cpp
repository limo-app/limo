...
OpenMwPluginDeployer::OpenMwPluginDeployer(const sfs::path &source_path,
  ...
  is_autonomous_ = true;
  plugin_regex_ = R"(.*\.[eE][sS][pPlLmM]$|.*\.[oO][mM][wW](addon|game)$)";
  plugin_file_line_regex =
      R"(^\s*(\*?)([^#]*\.(?:[eE][sS][pP]|[eE][sS][mM]|[oO][mM][wW](?:addon|game)))(\r?))";
...
void OpenMwPluginDeployer::unDeploy(
...
void OpenMwPluginDeployer::writePlugins() const {
  ...
  if (!in_file.is_open())
    throw std::runtime_error(
        std::format("Error: Could not open '{}'.", plugin_file_path.string()));
...
bool OpenMwPluginDeployer::initPluginFile() {
  ...
  if (!in_file.is_open())
    throw std::runtime_error(
        std::format("Error: Could not open '{}'.", config_file_path.string()));

  std::string line;
  std::regex plugin_regex(
      R"(^content=(.*?\.(?:[eE][sS][pP]|[eE][sS][mM]|[oO][mM][wW](?:addon|game))))");
  while (getline(in_file, line)) {
    std::smatch match;
